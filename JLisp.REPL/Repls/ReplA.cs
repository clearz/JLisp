...
namespace JLisp
{
    ...
    class ReplA : IProcess
    {
        ...
        public void Init()
        {
            ...
            _rootEnv.Set(new JlSymbol("eval"), new JlFunction(
                a => Eval(a[0], _rootEnv)));
            ...
            Re("(defmacro! or (fn* (& xs) (if (empty? xs) nil (if (= 1 (count xs)) (first xs) (let* (condvar (gensym)) `(let* (~condvar ~(first xs)) (if ~condvar ~condvar (or ~@(GetTail xs)))))))))");
